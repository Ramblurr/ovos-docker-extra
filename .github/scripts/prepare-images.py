...
def get_upstream_version(app, channel):
    latest_script = f"./images/{app}/ci/latest.sh"
...
if __name__ == "__main__":
    ...
    for app in changed_apps:
        ...
        with open(f"./images/{name}/metadata.json") as f:
        ...
        if isfile(f"./images/{name}/{channel}/Dockerfile"):
            app["chan_dockerfile"] = f"./images/{name}/{channel}/Dockerfile"
            app["chan_goss_config"] = f"./images/{name}/{channel}/goss.yaml"
        else:
            app["chan_dockerfile"] = f"./images/{name}/Dockerfile"
            app["chan_goss_config"] = f"./images/{name}/ci/goss.yaml"
